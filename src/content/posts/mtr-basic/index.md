...
## 如何判讀丟包：中間節點不回應，不等於連線丟包

路由器可能限制或降低 ICMP 回應的優先權，同時仍正常轉送流量。因此，不能只看到某一跳的 `Loss%` 很高，就判定該節點是故障來源。

以下是用來說明判讀方式的假設數據，並非實測紀錄：

| 節點                | Loss% | Snt | Avg   |
| ------------------- | ----- | --- | ----- |
| 第 1 跳：本地閘道   | 0%    | 100 | 1 ms  |
| 第 2 跳：中間路由器 | 80%   | 100 | 8 ms  |
| 第 3 跳：目的主機   | 0%    | 100 | 20 ms |

第 2 跳有 80% 未回應，但目的主機仍回覆全部探測。這份結果不支持「經過第 2 跳的流量都遺失 80%」；中間節點對探測封包限速是可能的解釋。

如果後續多個節點與目的主機也持續出現丟包，才值得進一步調查。仍需考慮回程路由、目的端限速與負載平衡，不能把第一個顯示丟包的節點直接當成故障位置。

排查 HTTPS 連線時，可比較預設 ICMP 與目的埠 443 的 TCP 探測。以下使用 `example.com` 作為示範，實際操作請替換成要診斷的主機：

```bash
mtr -rw -c 100 example.com
mtr -rw -c 100 --tcp --port 443 example.com
```

MTR 的 TCP 探測不會完成 TLS 握手或 HTTP 請求。若路徑看起來正常但網站仍無法開啟，可接著用 [curl 檢查 TLS、HTTP 狀態與各階段耗時](/posts/curl-guide/)。

## MTR 使用技巧總結

MTR 是網路診斷的瑞士刀，結合了 ping 的持續監控和 traceroute 的路由追蹤功能。掌握 MTR 能讓您：

- **縮小問題範圍**：比較中間節點與目的端的丟包、延遲，再搭配其他證據判讀
- **持續監控網路品質**：即時觀察網路狀態變化
- **生成專業報告**：匯出詳細數據供分析或呈報
